...
__device__ uint8_t grayToColor(const float32_t gray)
{
    float32_t colorF = 0.0f;

    if (gray <= -0.75f) {
        colorF = 0.0f;
    } else if (gray <= -0.25f) {
        colorF = 255.0f * ((gray + 0.75f) / 0.5f);
    } else if (gray <= 0.25f) {
        colorF = 255.0f;
    } else if (gray <= 0.75f) {
        colorF = 255.0f * (((0.25f - gray) / 0.5f) + 1.0f);
    } else {
        colorF = 0.0f;
    }

    const uint8_t color = static_cast<uint8_t>(colorF);

    return color;
}

__global__ void mixDispConfKernel(uint8_t* colorOut, size_t pitchColor, uint8_t* confidence,
                                  size_t pitchConfidence, const uint32_t width, const uint32_t height,
                                  bool showInvalid)
{
    ...
    if (val > 0) val = 1;
    if (showInvalid && (confidence[tidy*pitchConfidence + tidx] == 1)) val = 255;
...
__global__ void colorCodeKernel(uint8_t* color, size_t pitchOut, uint8_t* gray, size_t pitchIn,
                                const uint32_t width, const uint32_t height, const float32_t scaleFactor)
{
    const uint32_t tidx = blockDim.x * blockIdx.x + threadIdx.x;
    const uint32_t tidy = blockDim.y * blockIdx.y + threadIdx.y;
    ...
    float32_t normGray = static_cast<float32_t>(gray[tidy*pitchIn + tidx]) / 255.0f;
    normGray *= scaleFactor;
    color[tidy*pitchOut*1 + 4*tidx+0] = grayToColor(1.5f - normGray);
    color[tidy*pitchOut*1 + 4*tidx+1] = grayToColor(1.0f - normGray);
    color[tidy*pitchOut*1 + 4*tidx+2] = grayToColor(0.5f - normGray);
...
void colorCode(dwImageCUDA *colorOut, dwImageCUDA disparity, const float32_t scaleFactor)
{
    ...
    colorCodeKernel <<<dim3(iDivUp(disparity.prop.width, numThreads.x),
            ...
            numThreads >>>(reinterpret_cast<uint8_t*>(colorOut->dptr[0]), colorOut->pitch[0],
                           ...
                           disparity.prop.width, disparity.prop.height, scaleFactor);
...
__global__ void createAnaglyphRKernel(uint8_t* color, size_t pitchOut, uint8_t* grayLeft, uint8_t* grayRight,
                               size_t pitchIn, const uint32_t width, const uint32_t height)
{
    const uint32_t tidx = blockDim.x * blockIdx.x + threadIdx.x;
    const uint32_t tidy = blockDim.y * blockIdx.y + threadIdx.y;
...
void createAnaglyph(dwImageCUDA anaglyph, dwImageCUDA inputLeft, dwImageCUDA inputRight)
{
    ...
    dim3 numThreads = dim3(32, 4, 1);
    if ((inputLeft.prop.pxlFormat == DW_IMAGE_R) || (inputLeft.prop.pxlFormat == DW_IMAGE_YUV420)) {
